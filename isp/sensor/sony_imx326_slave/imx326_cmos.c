...
#define IMX326_FULL_LINES_STD_FPGA_COMP 0

const IMX326_VIDEO_MODE_TBL_S g_astImx326ModeTbl[IMX326_MODE_BUTT] = {
    {24000000, 0xC8, 0xC3500, 4000, 30,  "3072x1728_6CH_12BIT_30FPS"}, //MODE0
    {24000000, 0xaf, 0xC3820, 4576, 29.97,  "2592x1944_6CH_12BIT_29.97FPS"}, //MODE01
};

ISP_SLAVE_SNS_SYNC_S gstImx326Sync[ISP_MAX_DEV_NUM];
...
HI_BOOL g_bStart[ISP_MAX_DEV_NUM] = {0,0};
...
static HI_S32 cmos_get_ae_default(ISP_DEV IspDev,AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    u32Fll = g_astImx326ModeTbl[g_apstSnsState[IspDev]->u8ImgMode].u32VertiLines;
    f32maxFps = g_astImx326ModeTbl[g_apstSnsState[IspDev]->u8ImgMode].f32MaxFps;
...
static HI_VOID cmos_fps_set(ISP_DEV IspDev,HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    f32maxFps = g_astImx326ModeTbl[g_apstSnsState[IspDev]->u8ImgMode].f32MaxFps;
    u32Lines = g_astImx326ModeTbl[g_apstSnsState[IspDev]->u8ImgMode].u32VertiLines * f32maxFps / f32Fps;
    ...
    gstImx326Sync[IspDev].u32VsTime = g_astImx326ModeTbl[g_apstSnsState[IspDev]->u8ImgMode].u32InckPerVs * f32maxFps / f32Fps;
    g_apstSnsState[IspDev]->u32FLStd = g_astImx326ModeTbl[g_apstSnsState[IspDev]->u8ImgMode].u32VertiLines * f32maxFps / f32Fps;

    g_apstSnsState[IspDev]->astRegsInfo[0].stSlvSync.u32SlaveVsTime = gstImx326Sync[IspDev].u32VsTime;
    g_apstSnsState[IspDev]->u32FLStd = (g_apstSnsState[IspDev]->u32FLStd > FULL_LINES_MAX) ? FULL_LINES_MAX : g_apstSnsState[IspDev]->u32FLStd;
    ...
    AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	pstAeSnsDft->u32FullLines = g_apstSnsState[IspDev]->au32FL[0];
    gstImx326Sync[IspDev].u32VsTime = g_astImx326ModeTbl[g_apstSnsState[IspDev]->u8ImgMode].u32InckPerHs * u32FullLines;
    g_apstSnsState[IspDev]->astRegsInfo[0].stSlvSync.u32SlaveVsTime = gstImx326Sync[IspDev].u32VsTime;
...
static HI_VOID cmos_gains_update(ISP_DEV IspDev,HI_U32 u32Again, HI_U32 u32Dgain)
{
    ...
    g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[4].u32Data = (u32Dgain & 0x7);
...
static HI_VOID cmos_set_pixel_detect(ISP_DEV IspDev,HI_BOOL bEnable)
{
    ...
    if (bEnable)
    {
        switch(g_apstSnsState[IspDev]->u8ImgMode) 
        {
            case IMX326_MODE0:
            case IMX326_MODE1:
                gstImx326Sync[IspDev].u32VsTime = ((g_astImx326ModeTbl[g_apstSnsState[IspDev]->u8ImgMode].u32InckPerVs) * f32maxFps)/5;
                break;
            default:
    ...
    {
        gstImx326Sync[IspDev].u32VsTime = (g_astImx326ModeTbl[g_apstSnsState[IspDev]->u8ImgMode].u32InckPerVs);
...
static HI_VOID cmos_set_wdr_mode(ISP_DEV IspDev,HI_U8 u8Mode)
{
    ...
    {
        case WDR_MODE_NONE:
            g_apstSnsState[IspDev]->u32FLStd = 4000+IMX326_FULL_LINES_STD_FPGA_COMP;
...
static HI_U32 cmos_get_sns_regs_info(ISP_DEV IspDev,ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((HI_FALSE == g_apstSnsState[IspDev]->bSyncInit) || (HI_FALSE == pstSnsRegsInfo->bConfig))
    {
        ...
        g_apstSnsState[IspDev]->astRegsInfo[0].unComBus.s8SspDev.bit4SspDev = g_aunImx326BusInfo[IspDev].s8SspDev.bit4SspDev;
        g_apstSnsState[IspDev]->astRegsInfo[0].unComBus.s8SspDev.bit4SspCs = g_aunImx326BusInfo[IspDev].s8SspDev.bit4SspCs;
        g_apstSnsState[IspDev]->astRegsInfo[0].u8Cfg2ValidDelayMax = 2;
        g_apstSnsState[IspDev]->astRegsInfo[0].u32RegNum = 7;
        ...
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[0].u32RegAddr = IMX326_SHR_L;
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[1].u8DelayFrmNum = 0;
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[1].u32RegAddr = IMX326_SHR_H;

        // gain related
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[2].u32RegAddr = IMX326_PGC_L;
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[2].u8DelayFrmNum = 1;
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[3].u32RegAddr = IMX326_PGC_H;
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[3].u8DelayFrmNum = 1;
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[4].u32RegAddr = IMX326_DGAIN;
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[4].u8DelayFrmNum = 1;

        //svr registers
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[5].u32RegAddr = IMX326_SVR_L;
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[5].u32Data = 0;
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[5].u8DelayFrmNum = 0;
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[6].u32RegAddr = IMX326_SVR_H;
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[6].u8DelayFrmNum = 0;
        g_apstSnsState[IspDev]->astRegsInfo[0].astSspData[6].u32Data = 0;       
...
static HI_S32 cmos_set_image_mode(ISP_DEV IspDev,ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if ( IMX326_RES_IS_3072x1728_5M30fps(u32W, u32H))
    {
        u8SensorImageMode = IMX326_MODE0;
    }
    else if (IMX326_RES_IS_2592x1944_5M30fps(u32W, u32H))
    {
        u8SensorImageMode = IMX326_MODE1;
    } 
    else
    {
        return -1;
...
static HI_VOID sensor_global_init(ISP_DEV IspDev)
{
    ...
    g_apstSnsState[IspDev]->u32FLStd= 4000+IMX326_FULL_LINES_STD_FPGA_COMP;
    g_apstSnsState[IspDev]->u8ImgMode = IMX326_MODE0;
...
{
    g_aunImx326BusInfo[IspDev].s8SspDev.bit4SspDev = unSNSBusInfo.s8SspDev.bit4SspDev;
    g_aunImx326BusInfo[IspDev].s8SspDev.bit4SspCs  = unSNSBusInfo.s8SspDev.bit4SspCs;
...
ISP_SNS_OBJ_S stSnsImx326SlaveObj = 
{
