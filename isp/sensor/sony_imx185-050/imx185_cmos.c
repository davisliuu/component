...
//lxcadd
extern void sensor_init(ISP_DEV IspDev);
extern void sensor_exit(ISP_DEV IspDev);
extern int sensor_write_register(unsigned int addr, unsigned char data);
extern int sensor_read_register(unsigned int addr);

//extern int my_sensor_write_register(ISP_DEV IspDev, int addr, int data);
//#define  my1_sensor_write_register(IspDev,addr,data) sensor_write_register(addr,data);
//#define  my_sensor_write_register my1_sensor_write_register
//lxcend
...
static HI_S32 cmos_get_ae_default(AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    {
        printf("null pointer when get ae default value11111!\n");
        return -1;
    }
...
static HI_S32 cmos_get_ae_default(ISP_DEV IspDev,AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    if (HI_NULL == pstAeSnsDft)
    {
        printf("null pointer when get ae default value22222!\n");
        return -1;
    }
...
//	printf("  cmos_gains_update u32Again = 0x%x  u32Dgain = 0x%x \n",u32Again,u32Dgain);
...
printf("cmos_get_isp_default 11111111111111111\n");
    if (HI_NULL == pstDef)
    {
    ...
    switch (genSensorMode)
    {
        ...
        case WDR_MODE_NONE:           
            ...
            memcpy(&pstDef->stRgbSharpen, &g_IspDft[0].stRgbSharpen, sizeof(ISP_CMOS_RGBSHARPEN_S)); //lxc
	//		memcpy(&pstDef->stYuvSharpen, &g_stIspYuvSharpen, sizeof(ISP_CMOS_YUV_SHARPEN_S)); //lxcadd
      ...
      //lxc      memcpy(&pstDef->stRgbSharpen, &g_IspDft[1].stRgbSharpen, sizeof(ISP_CMOS_RGBSHARPEN_S)); 		
...
static ISP_CMOS_AGC_TABLE_S g_stIspAgcTable =
{
    ...
    /* sharpen_alt_d */
//lxc    {0x38,0x38,0x36,0x32,0x2b,0x26,0x20,0x1c,0x1a,0x16,0x14,0x12,0x12,0x12,0x12,0x12},
        
    /* sharpen_alt_ud */
//lxc    {0x32,0x32,0x30,0x30,0x30,0x2a,0x2a,0x2a,0x26,0x26,0x20,0x16,0x12,0x12,0x12,0x12},
        
    /* snr_thresh Max=0x54 */
    {0x08,0x0a,0x0f,0x12,0x16,0x1a,0x22,0x28,0x2e,0x36,0x3a,0x40,0x40,0x40,0x40,0x40},
        
    /* demosaic_lum_thresh */
//lxc    {0x58,0x58,0x56,0x4e,0x46,0x3a,0x30,0x28,0x24,0x20,0x20,0x20,0x20,0x20,0x20,0x20},
...
//lxc    {0x58,0x58,0x56,0x4e,0x46,0x36,0x36,0x34,0x34,0x32,0x30,0x20,0x20,0x20,0x20,0x20}
};
...
static HI_U32 cmos_get_isp_default(ISP_DEV IspDev,ISP_CMOS_DEFAULT_S *pstDef)
{
    ...
    switch (genSensorMode)
    {
        ...
        case WDR_MODE_NONE:
            ...
            memcpy(&pstDef->stRgbSharpen, &g_stIspRgbSharpen, sizeof(ISP_CMOS_RGBSHARPEN_S)); //lxc
        break;
        case WDR_MODE_BUILT_IN:
            ...
            memcpy(&pstDef->stNoiseTbl, &g_stIspNoiseTable, sizeof(ISP_CMOS_NOISE_TABLE_S));
       //lxc     memcpy(&pstDef->stAgcTbl, &g_stIspAgcTableBuiltInWDR, sizeof(ISP_CMOS_AGC_TABLE_S));
        ...
        //lxc    memcpy(&pstDef->stRgbSharpen, &g_stIspRgbSharpen, sizeof(ISP_CMOS_RGBSHARPEN_S));
        break;
    }
...
printf("cmos_get_isp_black_level 11111111111111111111111\n");
...
printf("cmos_set_pixel_detect 11111111111111111111111\n");
    ...
    if (bEnable) /* setup for ISP pixel calibration mode */
    {
        ...
        sensor_write_register(VMAX_ADDR, 0x5E); //sensor_write_register
...
static HI_VOID cmos_set_wdr_mode(ISP_DEV IspDev,HI_U8 u8Mode)
{
 ...
 printf("cmos_set_wdr_mode  111111111111\n");
    switch(u8Mode)
    {
        case WDR_MODE_NONE:
...
//printf("cmos_get_sns_regs_info 1111111111111!\n");
    if (HI_FALSE == bInit)
    {
...
{
    HI_S32 s32Ret;
//lxc20180810     ALG_LIB_S stLib;
//lxc    ISP_DEV IspDev=0;
    ISP_SENSOR_REGISTER_S stIspRegister;
    AE_SENSOR_REGISTER_S  stAeRegister;
    AWB_SENSOR_REGISTER_S stAwbRegister;


printf("sensor register callback function 111111111111111111111111111111111!\n");
    cmos_init_sensor_exp_function(&stIspRegister.stSnsExp);
//lxc    s32Ret = HI_MPI_ISP_SensorRegCallBack(IspDev, IMX185_ID, &stIspRegister);
	s32Ret = HI_MPI_ISP_SensorRegCallBack(0, IMX185_ID, &stIspRegister);
    if (s32Ret)
    {
...
printf("sensor register callback function 2222222222222222222222222222222!\n");
//lxc20180810    stLib.s32Id = 0;
//lxc20180810    strncpy(stLib.acLibName, HI_AE_LIB_NAME, sizeof(HI_AE_LIB_NAME));
    cmos_init_ae_exp_function(&stAeRegister.stSnsExp);
//    s32Ret = HI_MPI_AE_SensorRegCallBack(IspDev, &stLib, IMX185_ID, &stAeRegister);
//lxc20180809	    s32Ret = HI_MPI_AE_SensorRegCallBack(0, &stLib, IMX185_ID, &stAeRegister);
	s32Ret = HI_MPI_AE_SensorRegCallBack(0, pstAeLib, IMX185_ID, &stAeRegister);
    if (s32Ret)
    {
...
printf("sensor register callback function 33333333333333333333333333333333333!\n");
//lxc20180810    stLib.s32Id = 0;
//lxc20180810    strncpy(stLib.acLibName, HI_AWB_LIB_NAME, sizeof(HI_AWB_LIB_NAME));
    cmos_init_awb_exp_function(&stAwbRegister.stSnsExp);
//lxc    s32Ret = HI_MPI_AWB_SensorRegCallBack(IspDev, &stLib, IMX185_ID, &stAwbRegister);
//lxc20180810	s32Ret = HI_MPI_AWB_SensorRegCallBack(0, &stLib, IMX185_ID, &stAwbRegister);
	s32Ret = HI_MPI_AWB_SensorRegCallBack(0, pstAwbLib, IMX185_ID, &stAwbRegister);
    if (s32Ret)
    {
...
printf("sensor register callback function 4444444444444444444444444444!\n");
    return 0;
}
...
//lxc    ISP_DEV IspDev=0;
    ...
    s32Ret = HI_MPI_ISP_SensorUnRegCallBack(0, IMX185_ID);
//lxc s32Ret = HI_MPI_ISP_SensorUnRegCallBack(IspDev, IMX185_ID);
    if (s32Ret)
    {
    ...
    s32Ret = HI_MPI_AE_SensorUnRegCallBack(0, &stLib, IMX185_ID);
//lxc  s32Ret = HI_MPI_AE_SensorUnRegCallBack(IspDev, &stLib, IMX185_ID);
    if (s32Ret)
    {
    ...
    s32Ret = HI_MPI_AWB_SensorUnRegCallBack(0, &stLib, IMX185_ID);
//lxc s32Ret = HI_MPI_AWB_SensorUnRegCallBack(IspDev, &stLib, IMX185_ID);
    if (s32Ret)
    {
...
int my_sensor_write_register(ISP_DEV IspDev,int addr, int data)
{
	return sensor_write_register(addr, data);
}
int my_sensor_read_register(ISP_DEV IspDev,int addr)
{
	return sensor_read_register(addr);
}
//lxc
ISP_SNS_OBJ_S stSnsImx185Obj = 
{
    .pfnRegisterCallback    = sensor_register_callback,
    .pfnUnRegisterCallback  = sensor_unregister_callback,
 //   .pfnStandby             = imx185_standby,
 //   .pfnRestart             = imx185_restart,
    .pfnWriteReg            = my_sensor_write_register, //my_sensor_write_register
    .pfnReadReg             = my_sensor_read_register						//imx185_read_register,
 //   .pfnSetBusInfo          = imx185_set_bus_info
};
//lxcend
