 ...
 *----------------------------------------------*/
ISP_SYNC_TSK_CTX_S g_astIspSyncTskCtx[2];
/*----------------------------------------------*
 * constants                                    *
 *----------------------------------------------*/
...
HI_S32 ispSyncTskFIndAndExecute(ISP_DEV IspDev, struct osal_list_head *head)
{
    ...
    if (!osal_list_empty(head))
    {
        osal_list_for_each_safe(pos, next, head)
        {
            ...
            {
                ISP_DRV_StaKernelGet(IspDev, pstSyncTskNode->pstFocusStat);
            }
        }     
    }   
...
static void workQueueHandler(struct osal_work_struct *pstWorker)
{
   ...
   ISP_SYNC_TSK_CTX_S *pstSyncTsk = osal_container_of((void*)pstWorker, ISP_SYNC_TSK_CTX_S, worker);
    ...
    ispSyncTskFIndAndExecute(pstSyncTsk->IspDev, &pstSyncTsk->workqueue_list.head);  
...
HI_S32 IspSyncTaskProcess(ISP_DEV dev)
{
    ...
    {
        ispSyncTskFIndAndExecute(pstSyncTsk->IspDev, &pstSyncTsk->hwirq_list.head);
    }
