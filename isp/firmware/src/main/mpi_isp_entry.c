 ...
 * static functions 
 ****************************************************************************/
static HI_BOOL ISP_GetVersion(ISP_DEV IspDev)
{
    HI_S32 s32Ret;
...
HI_S32 HI_MPI_ISP_MemInit(ISP_DEV IspDev)
{
    ...
    if (HI_TRUE != ISP_GetVersion(IspDev))
    {
        return HI_ERR_ISP_ILLEGAL_PARAM;
    }

    /* 2. creat extern registers */
    s32Ret = VReg_Init(ISP_VREG_BASE(IspDev), ISP_VREG_SIZE);
    ...
    pstIspCtx->enIspMode = 0;
    if (ioctl(g_as32IspFd[IspDev], ISP_MODE_INFO, &pstIspCtx->enIspMode))
    {
        return HI_ERR_ISP_ILLEGAL_PARAM;
    }
    ...
    hi_ext_top_isp_working_mode_write(IspDev, 0);
...
HI_S32 HI_MPI_ISP_Init(ISP_DEV IspDev)
{
    ...
    pstIspCtx->stIspParaRec.bWDRCfg = hi_ext_top_wdr_cfg_read(IspDev);
    ISP_CHECK_ISP_WDR_CFG(IspDev);
    
    pstIspCtx->stIspParaRec.bPubCfg = hi_ext_top_pub_attr_cfg_read(IspDev);
    ...
    s32Ret = ISP_FrameInfoInit(IspDev, &pstIspCtx->stFrameInfoCtrl);
    if (HI_SUCCESS != s32Ret)
    {
        goto fail3;
    }
...
#endif
    stSnsImageMode.u16Width = hi_ext_top_sensor_width_read(IspDev);
    stSnsImageMode.u16Height = hi_ext_top_sensor_height_read(IspDev);
    ...
    ISP_ExtRegsDefault(IspDev);
    ISP_RegsDefault(IspDev);
    ...
    /* 10. set snap default attr to kernel */
    s32Ret = ISP_SnapDefaultSet(IspDev);
    if (HI_SUCCESS != s32Ret)
    {
        ISP_TRACE(HI_DBG_ERR, "ISP[%d] set snap default attr to kernel failed!\n", IspDev);
        goto fail4;
    }

    /* 11. init isp global variables */
    ...
    return HI_SUCCESS;
fail4:  
    ISP_FrameInfoExit(IspDev, &pstIspCtx->stFrameInfoCtrl);
fail3:      
...
HI_S32 HI_MPI_ISP_Exit(ISP_DEV IspDev)
{
    ...
    /* 8. release vregs */
    VReg_Munmap(ISP_REG_BASE(IspDev), VI_ISP_REG_SIZE);
    VReg_Exit(ISP_VREG_BASE(IspDev), ISP_VREG_SIZE);
...
/*****************************************************************************
 Prototype       : HI_MPI_ISP_SetMode
 Description     : isp working mode to firmware
 Input           : enIspMode
 Output          : None
 Return Value    : 
 Process         : 
...
HI_S32 HI_MPI_ISP_SetMode(ISP_DEV IspDev, const ISP_MODE_E enIspMode)
{
    ...
    if (HI_TRUE == pstIspCtx->stIspParaRec.bInit)
    {
        ISP_TRACE(HI_DBG_ERR, "ISP[%d] can NOT set ISPMode dynamically!\n", IspDev);
        return HI_ERR_ISP_ILLEGAL_PARAM;
    }

    if (enIspMode >= ISP_MODE_BUTT)
    {
        ISP_TRACE(HI_DBG_ERR, "Invalid ISP[%d] mode %d!\n", IspDev, enIspMode);
        return HI_ERR_ISP_ILLEGAL_PARAM;
    } 

    pstIspCtx->enIspMode = enIspMode;
    if (ioctl(g_as32IspFd[IspDev], ISP_MODE_INFO, &pstIspCtx->enIspMode))
    {
        return HI_ERR_ISP_ILLEGAL_PARAM;
    }
    
    hi_ext_top_isp_working_mode_write(IspDev, pstIspCtx->enIspMode);
...
HI_S32 HI_MPI_ISP_GetMode(ISP_DEV IspDev, ISP_MODE_E *penIspMode)
{
    ...
    u8IspMode = hi_ext_top_isp_working_mode_read(IspDev);
    if(u8IspMode >= ISP_MODE_BUTT)
    {
        *penIspMode = ISP_MODE_BUTT;
    }
    else
    {
        *penIspMode = u8IspMode;
    }
...
HI_S32 HI_MPI_ISP_SetDCFInfo(ISP_DEV IspDev, const ISP_DCF_INFO_S *pstIspDCF)
{
    ...
    {
        pstIspCtx->stUpdateInfoCtrl.u32UpdateInfoPhyaddr = hi_ext_system_update_info_phyaddr_read(IspDev);

        pstIspCtx->stUpdateInfoCtrl.pstIspUpdateInfo = HI_MPI_SYS_Mmap(pstIspCtx->stUpdateInfoCtrl.u32UpdateInfoPhyaddr,
    ...
    pstIspCtx->stUpdateInfoCtrl.pstIspDCFConstInfo->u8MeteringMode          = pstIspDCF->u8MeteringMode;
...
HI_S32 HI_MPI_ISP_GetDCFInfo(ISP_DEV IspDev, ISP_DCF_INFO_S *pstIspDCF)
{
    ...
    pstIspDCF->u8MeteringMode           = pstIspCtx->stUpdateInfoCtrl.pstIspDCFConstInfo->u8MeteringMode;
    ...
    pstIspDCF->u32FNumber               = pstIspCtx->stUpdateInfoCtrl.pstIspUpdateInfo->u32FNumber;              
    pstIspDCF->u32MaxApertureValue      = pstIspCtx->stUpdateInfoCtrl.pstIspUpdateInfo->u32MaxApertureValue;
...
HI_S32 HI_MPI_ISP_SetFrameInfo(const FRAME_SUPPLEMENT_INFO_S *pstIspFrame)
{
    ISP_DEV IspDev = 0;
    HI_S32 s32Ret;
    ISP_CTX_S *pstIspCtx = HI_NULL;
    ...
    s32Ret = ioctl(g_as32IspFd[IspDev], ISP_FRAME_INFO_SET, pstIspFrame);
    if (s32Ret)
    {
        return s32Ret;
    }

    memcpy(pstIspCtx->stFrameInfoCtrl.pstIspFrame,pstIspFrame,sizeof(FRAME_SUPPLEMENT_INFO_S));
    return HI_SUCCESS;
}

HI_S32 HI_MPI_ISP_GetFrameInfo(FRAME_SUPPLEMENT_INFO_S *pstIspFrame)
{
    ISP_DEV IspDev = 0;
    ISP_CTX_S *pstIspCtx = HI_NULL;
    ...
    if (ioctl(g_as32IspFd[IspDev], ISP_FRAME_INFO_GET, pstIspFrame))
    {
        return HI_ERR_ISP_NOMEM;
    }
