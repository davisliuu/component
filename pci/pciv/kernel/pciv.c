...
static HI_VOID PcivCheckNotifyCnt(PCIV_CHN pcivChn, HI_U32 u32Index, HI_U32 u32Count)
{
    ...
    else
    {
        ...
        {
            PCIV_TRACE(HI_DBG_WARN, "Warnning: PcivChn:%d, ReadDone MsgSeq -> (%d,%d),bufindex:%d \n",
...
HI_S32 PCIV_ProcShow(osal_proc_entry_t *s)
{
    HI_S32 i;
    PCIV_CHANNEL_S  *pstPcivChn;
    PCIV_ATTR_S     *pstAttr;
...
//    osal_seq_printf(s, "\n[PCIV] Version: ["MPP_VERSION"], %s",proc_title);
    ...
    osal_seq_printf(s, "\n-----PCIV CHN STATUS------------------------------------------------------------\n");
    osal_seq_printf(s, "%6s"    "%8s"     "%8s"     "%8s"    "%8s"     "%8s"     "%8s"     "%8s"     "%12s"    "\n"
    ...
    for (pcivChn = 0; pcivChn < PCIV_MAX_CHN_NUM; pcivChn++)
    {
        ...
        osal_seq_printf(s, "%6d" "%8d" "%8d" "%8d" "%8d" "%8d" "%8d" "%8d" "%12s" "\n",
            pcivChn,
	...
	{
	    osal_seq_printf(s, "\n-----PCIV MSG STATUS------------------------------------------------------------\n");
	    osal_seq_printf(s, "%6s"    "%11s"     "%11s"     "%11s"     "%11s"     "%11s"     "%11s"    "\n"
	    ...
	    for (pcivChn = 0; pcivChn < PCIV_MAX_CHN_NUM; pcivChn++)
	    {
	        ...
	        osal_seq_printf(s, "%6d" "%11d" "%11d" "%11d" "%11d" "%11d" "%11d" "\n",
	            pcivChn,
