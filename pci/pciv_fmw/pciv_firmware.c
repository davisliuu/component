...
HI_S32 PCIV_FirmWareRecvPicAndSend(PCIV_CHN pcivChn, PCIV_PIC_S *pRecvPic)
{
    ...
    if (PCIV_BIND_VI == pRecvPic->enSrcType)
    {
        ...
        stViFrmInfo.stViFrmInfo.u32PoolId       = CALL_VB_Phy2Handle(CALL_VB_Phy2Handle(pFmwChn->au32PhyAddr[pRecvPic->u32Index]));
    ...
    else if (PCIV_BIND_VDEC == pRecvPic->enSrcType || PCIV_BIND_VO == pRecvPic->enSrcType)
    {
        ...
        stVideoFrmInfo.u32PoolId = CALL_VB_Phy2Handle(CALL_VB_Phy2Handle(pFmwChn->au32PhyAddr[pRecvPic->u32Index]));
        ...
        const VIDEO_FRAME_INFO_S *pstSrcFrame, VI_MIXCAP_STAT_S *pstMixCapState)
{
    ...
    if (s32Value == HI_SUCCESS && stRgnInfo.u32Num > 0)    
    {
        ...
        for (i=0; i<stRgnInfo.u32Num; i++)
        {
            ...
            stVgsOpt.stOsdOpt[i].stVgsOsd.u32PhyAddr    = stRgnInfo.pstRgnCommInfo->pstRgnComm->u32PhyAddr; 
...
HI_S32 PCIV_FmwInit(void *p)
{
    ...
    if ((CKFN_RGN()) && (CKFN_RGN_RegisterMod()))
    {
        ...
        stCapacity.stSize.stSizeMax.u32Width = RGN_OVERLAY_MAX_WIDTH;
        stCapacity.stSize.stSizeMax.u32Height = RGN_OVERLAY_MAX_HEIGHT;
        stCapacity.stSize.u32MaxArea = RGN_OVERLAY_MAX_WIDTH * RGN_OVERLAY_MAX_HEIGHT;
        ...
        stCapacity.stInvColor.stSizeCap.u32MaxArea = RGN_OVERLAY_MAX_WIDTH * RGN_OVERLAY_MAX_HEIGHT;
